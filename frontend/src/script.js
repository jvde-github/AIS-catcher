    ...
    settingsPanel.bind(
        ...
        { captions: {
          ...
          onChange: (k, v, el, staged) => {
            ...
            if (k === "shipcard_style") setTargetcardStyle(v);
...
function setTargetcardStyle(style) {
...
planecard.init({ fit: fitTargetcard });

ticker.init({
    buckets: bucketChips(),
    bucketHidden: (b) => filter.isActive() && filter.isHidden("bucket", b),
    resolveVessel: async (m) => {
        if (!shipsDB[m]) await fetchShips(false);
        return shipsDB[m]?.raw || null;
    },
