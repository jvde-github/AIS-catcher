/* ============================================================================
   ...
   Companion to components.css. An ES module: the viewer bundles it, the hub
   loads it through a module shim that publishes it as window.AISComponents.
   ========================================================================= */

// Decode one layer for textContent sinks; never insert the result as HTML.
...
export function fieldRows(mount, rows, opts) {
    opts = opts || {};
    var cells = {};
...
export var TOAST_TIME = { info: 5000, success: 5000, warning: 7000, error: 9000 };
...
export function toast(type, message, opts) {
    ...
    var id = opts.container || "toast-container";
    ...
    if (twin) {
        ...
        badge.textContent = "\u00d7" + n;
        if (onClose) twin._closers.push(onClose);
    ...
    function dismiss() {
        ...
        el._closers.forEach(function (fn) { fn(); });
    }
    close.addEventListener("click", dismiss);
    el._dismiss = dismiss;
    el._closers = onClose ? [onClose] : [];
    return dismiss;
}
...
/* ---------------------------------------------------------------------
   ...
   --------------------------------------------------------------------- */
export function placeMenu(menu, opts) {
    opts = opts || {};
...
export function dismissOnce(menu, onClose, opts) {
    ...
    function onClick(e) {
        if (opts.keepInside && menu.contains(e.target)) return;
...
/* --- flag ------------------------------------------------------------------
   A country flag as a <span class="flag fi fi-xx">. Needs the flag-icons
   stylesheet (npm: flag-icons) loaded by the host; without it the span is
   empty and harmless. `variant` picks a size treatment from components.css
   (flag-card, flag-tooltip, flag-table). */

export function flag(country, variant, title) {
    var span = document.createElement("span");
    if (!country) return span;
    span.className = "flag fi fi-" + String(country).toLowerCase() + (variant ? " " + variant : "");
    if (title) span.title = title;
    return span;
}

export function flagHTML(country, variant, title) {
    if (!country) return "<span></span>";
    var c = String(country).toLowerCase().replace(/[^a-z]/g, "");
    var t = title ? ' title="' + String(title).replace(/"/g, "&quot;") + '"' : "";
    return '<span class="flag fi fi-' + c + (variant ? " " + variant : "") + '"' + t + "></span>";
}

/* --- bucket chip -----------------------------------------------------------
   One counter chip: a sprite icon (sprites.css, positioned by core/filter.js
   BUCKETS/CLASSES) and a count the host writes to. */
...
export function bucketChip(b, countId) {
    ...
    if (b.style || b.pos) icon.setAttribute("style", b.style || chipStyle(b));
...
/* --- draggable -------------------------------------------------------------
   `handle` drags `target` (an absolutely positioned element); a move under
   15px is a click, not a drag. Dispatches "moved" { left, top } on target
   when a drag ends. */

export function draggable(handle, target) {
    var threshold = 15;
    var dragging = false;
    ...
    handle.addEventListener("pointerdown", function (e) {
        ...
        offX = startX - target.offsetLeft; offY = startY - target.offsetTop;
        ...
        var onUp = function (e) {
            if (dragging) {
                ...
                target.dispatchEvent(new CustomEvent("moved", {
                    detail: { left: parseInt(target.style.left) || 0, top: parseInt(target.style.top) || 0 },
                }));
            }
