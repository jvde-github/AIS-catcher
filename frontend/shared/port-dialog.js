...
export function createPortDialog(host) {
    return function openPorts(ports) {
        ...
        const load = async port => {
            ...
            dlg.setTitle(`To: ${port.label || port.code || 'Port'}`);
            count.textContent = '';
            ...
            try {
                ...
                count.title = 'Ships reporting this destination · latest reports first';
                if (!ships.length) {
                    results.innerHTML = '<p>No ships currently report this destination.</p>';
                    return;
                }
                ...
                const rows = ships.map(ship => {
                    ...
                    return `<tr data-mmsi="${id}"><td class="col-name" title="${name} · MMSI ${id}"><span class="table-name">${flagHTML(ship.country)}<button type="button" class="port-ship-link">${name}</button></span></td><td class="num col-spd">${speed}</td><td class="col-type">${icon}</td><td class="num col-last" title="${when}">${age}</td></tr>`;
                }).join('');
                results.innerHTML =
                    `<div class="tablecard_inner port-ship-list"><table><thead><tr><th scope="col" class="col-name">Name</th><th scope="col" class="num col-spd">Spd <span class="dim">kts</span></th><th scope="col" class="col-type">Type</th><th scope="col" class="num col-last" title="Last report">Last</th></tr></thead><tbody>${rows}</tbody></table></div>`;
            } catch (_) {
