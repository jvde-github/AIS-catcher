...
import { sanitizeString, formatTime, formatDateTime } from './core/text.js';
...
export const MAP_MARKER_RADIUS = 9;
export const MAP_MARKER_FILL_ALPHA = 0.9;
export const MAP_MARKER_BORDER_ALPHA = 0.5;
...
function disc(ctx, cat, cx, cy, R, [r, g, b], fade, ring = `rgba(255, 255, 255, ${MAP_MARKER_BORDER_ALPHA * fade})`) {
    ctx.beginPath();
    ctx.arc(cx, cy, R, 0, 2 * Math.PI);
    ctx.fillStyle = `rgba(${r}, ${g}, ${b}, ${MAP_MARKER_FILL_ALPHA * fade})`;
    ctx.fill();
...
export function discCanvas(cat, rgb, fade = 1, ring) {
    ...
    const R = MAP_MARKER_RADIUS, SIZE = 2 * (R + 2);
...
export function badgeCanvas(cat, rgb, fade = 1) {
    ...
    const [r, g, b] = rgb;
    ctx.strokeStyle = `rgba(${r}, ${g}, ${b}, ${MAP_MARKER_FILL_ALPHA * fade})`;
    ctx.lineWidth = 1.5;
    ctx.beginPath();
    ctx.moveTo(c, c);
    ctx.lineTo(cx, cy);
