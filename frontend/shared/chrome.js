/* ============================================================================
   ...
   Copied by build-html.sh to every served tree, like components.js, and loaded
   as a plain script exposing one global.
...
window.AISChrome = (function () {
    "use strict";

    var EDGES = ["top", "right", "bottom", "left"];
    var fallback = null;   // host hook for browsers without @property support
    var obstacles = [];    // [{ selector, edge }] floating over the free box
    var freeCache = [];    // one entry per parent asked about, cleared together

    function read(name, el) {
        var px = parseFloat(getComputedStyle(el || document.body).getPropertyValue(name));
    ...
    function insets(el) {
        var out = {};
        EDGES.forEach(function (edge) { out[edge] = inset(edge, el); });
        return out;
    }

    function gap() {
        var px = parseFloat(getComputedStyle(document.documentElement).getPropertyValue("--size-map-gap"));
        return isNaN(px) ? 10 : px;
    }
    ...
    function clamp(box, w, h, left, top) {
    ...
    function alongCourse(box, w, h, at, distance, angle) {
        var a = angle || 0;
    ...
    function beside(view, w, h, at, opts) {
        opts = opts || {};
    ...
    return {
        configure: function (opts) {
            fallback = (opts && opts.fallback) || null;
            obstacles = (opts && opts.obstacles) || [];
            freeCache = [];
        },
        invalidate: function () { freeCache = []; },
        inset: inset,
        insets: insets,
        gap: gap,
