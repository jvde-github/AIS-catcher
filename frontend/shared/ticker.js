...
export function create(opts) {
    ...
    function bindTouch() {
        ...
        const end = (e) => {
            const t = e.changedTouches[0] || { clientX: x0, clientY: y0 };
            ...
            slot.dragged = moved;
            // Resume automatic reading after touch; otherwise .held disables
            // the animation until another event happens to replace this one.
            release();
            const pan = panIfClipped();
            dwell = Math.max(dwellFor(slot.event || {}), pan * 1000 + 2000);
            schedule(dwell);
        };
    ...
    function measure() {
        ...
        if (slot?.event && isEnabled() && !slot.text.classList.contains("held")) {
            const overflow = Math.max(0, slot.scroller.getBoundingClientRect().width - slot.text.getBoundingClientRect().width);
            if (Math.abs(overflow - slot.overflow) > 0.5) {
                const pan = panIfClipped();
                dwell = Math.max(dwellFor(slot.event), pan * 1000 + 2000);
                if (!fadeTimer) schedule(dwell);
            }
        }
    }
    ...
    function show(event) {
        ...
        // The age consumes space in the same flex row. Set it before measuring,
        // including when the previous event had a shorter age label.
        showAge(event);
        release();
        const pan = panIfClipped();
    ...
    observer.observe(slot.text);
    observer.observe(slot.scroller);
