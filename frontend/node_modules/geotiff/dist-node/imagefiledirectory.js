...
exports.ImageFileDirectoryParser = exports.ImageFileDirectory = void 0;
...
function getValues(outValues = null, readMethod, dataSlice, fieldType, count, offset, isArray = false) {
    ...
    const isRational = (fieldType === globals_js_1.fieldTypes.RATIONAL || fieldType === globals_js_1.fieldTypes.SRATIONAL);
    // normal fields
    if (!isRational) {
...
class DeferredArray {
    ...
    constructor(source, arrayOffset, littleEndian, fieldType, length) {
        ...
        this.itemSize = (0, globals_js_1.getFieldTypeSize)(fieldType);
    ...
    async loadAll() {
        if (!this.fullFetchPromise) {
            this.fullFetchPromise = this.source.fetch([{
                ...
                const dataSlice = new dataslice_js_1.default(data[0], this.arrayOffset, true, false);
        ...
        if ((this.maskBitmap[byteIndex] & bitMask) === 0) {
            if (!this.fetchIndexPromises.has(index)) {
                const fetchPromise = this.source.fetch([{
                    ...
                    const dataSlice = new dataslice_js_1.default(data[0], this.arrayOffset + (index * this.itemSize), true, false);
...
class ImageFileDirectory {
    ...
    hasTag(tagIdentifier) {
        const tag = (0, globals_js_1.resolveTag)(tagIdentifier);
    ...
    getValue(tagIdentifier) {
        ...
        if (this.deferredFields.has(tag) || this.deferredArrays.has(tag)) {
            const tagDef = globals_js_1.tagDefinitions[tag];
    ...
    parseGeoKeyDirectory() {
        ...
        for (let i = 4; i <= rawGeoKeyDirectory[3] * 4; i += 4) {
            const key = ( /** @type {Record<number, import('./globals.js').GeoKeyName>} */(globals_js_1.geoKeyNames))[rawGeoKeyDirectory[i]];
    ...
    toObject() {
        ...
        for (const [tag, value] of this.actualizedFields.entries()) {
            const tagDefinition = typeof tag === 'number' ? globals_js_1.tagDefinitions[tag] : undefined;
...
exports.ImageFileDirectory = ImageFileDirectory;
/**
...
class ImageFileDirectoryParser {
    ...
    async parseFileDirectoryAt(offset) {
        ...
        for (let entryCount = 0; entryCount < numDirEntries; i += entrySize, ++entryCount) {
            ...
            const isArray = globals_js_1.tagDefinitions[fieldTag]?.isArray;
            const eager = globals_js_1.tagDefinitions[fieldTag]?.eager || this.eager;
            ...
            if (fieldTypeLength * typeCount <= (this.bigTiff ? 8 : 4)) {
                ...
                const length = (0, globals_js_1.getFieldTypeSize)(fieldType) * typeCount;
...
exports.ImageFileDirectoryParser = ImageFileDirectoryParser;
