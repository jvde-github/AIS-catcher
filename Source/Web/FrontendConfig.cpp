  ...
  try {
    ...
    if (firstline.length() > 2 && firstline[0] == '/' && firstline[1] == '/')
      ...
      throw std::runtime_error(
          "Plugin does not start with // followed by JSON description.");
    ...
    Info() << "Adding plugin (" + arg + "). Description: \"" << description
    ...
    if (version < 3 || version > 5)
      throw std::runtime_error("Version not supported, expected 3..5, got " +
                               std::to_string(version));
    if (version == 3)
      Warning() << "Plugin \"" << arg
                << "\" declares version 3 (pre-CSP). It may render but "
                   "inline-string handlers will not run under strict CSP. "
                   "Consider updating to version 5.";
    loaded_list.emplace_back(arg, version);
    std::string safe_arg = JSON::Writer::escape(arg);
    code += "\ntry{\n" + s +
            "\n} catch (error) {\nshowDialog(\"Error in Plugin \" + " +
            safe_arg +
            ", \"Plugins contain error: \" + error + \"</br>Consider updating "
            "plugins or disabling them.\"); }\n";
  } catch (const std::exception &e) {
    Warning() << "Server: Plugin \"" + arg + "\" ignored - JS plugin error : "
              << e.what();
...
void PluginStore::addDir(const std::string &dir) {
  ...
  const std::vector<std::string> &files_js =
      Util::Helper::getFilesWithExtension(dir, ".pjs");
  for (const auto &f : files_js)
    addPlugin(f);

  const std::vector<std::string> &files_ss =
      Util::Helper::getFilesWithExtension(dir, ".pss");
...
// An unreadable about file is reported, not fatal: the viewer serves without it
void PluginStore::setAbout(const std::string &path) {
  ...
  try {
    ...
    Error() << "Server: about context not loaded: " << e.what();
    about.clear();
    about_present = false;
  }
}
...
void FrontendConfig::setStation(const std::string &name) { station = name; }

void FrontendConfig::setReceivers(
    const std::vector<std::unique_ptr<ReceiverTracker>> &states) {
  receivers.clear();
...
std::string FrontendConfig::json(const PluginStore &plugins,
                                 std::string *revisionOut) const {
  std::string out;

  // 1. Emit the structured config blob. Single source of truth for all
  //    server-side state the frontend needs at startup.
  // Also served independently by api/config.json.
  const auto emit = [&](const std::string &version) {
    out.clear();
    JSON::Writer w(out);
    w.beginObject();
    if (!version.empty())
      w.kv("config_version", version);
    w.kv("plugin_version", plugins.version())
        .kv("about_version", Util::SHA256::hex(plugins.getAbout()))
        .key("maps")
        .beginArray();
    for (const auto &source : maps)
      w.raw_val(source);
    w.endArray().key("restart_required").beginArray();
    for (const auto &reason : restart_required)
      w.val(reason);
    w.endArray()
        .key("build")
        .beginObject()
    ...
    w.endArray().key("plugins").beginObject().key("loaded").beginArray();
  ...
  emit("");
  const auto version = Util::SHA256::hex(out + plugins.getAbout());
  if (revisionOut)
    *revisionOut = version;
  emit(version);
  return out;
...
std::string FrontendConfig::render(const PluginStore &plugins) const {
  return "window.__SERVER_CONFIG__ = " + json(plugins) + ";\n" +
         plugins.getCode() + "}\n";
}

const std::string &PluginStore::version() const {
  if (revision.empty())
    revision = Util::SHA256::hex(code + stylesheets);
  return revision;
}
