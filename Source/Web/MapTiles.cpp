...
MapTiles::MapTiles() : minZoom(0), maxZoom(18) {
  static std::atomic<int> next_layer{0};
...
bool MBTilesSupport::open(const std::string &filename) {
  if (sqlite3_open_v2(filename.c_str(), &db, SQLITE_OPEN_READONLY, nullptr) !=
      SQLITE_OK)
    return false;
...
MBTilesSupport::getTile(int z, int x, int y, std::string &contentType) {
  contentType.clear();
  tileData.clear();
  ...
  sqlite3_stmt *stmt;
  const char *query = "SELECT tile_data FROM tiles WHERE zoom_level = ? AND "
                      "tile_column = ? AND tile_row = ?";

  if (sqlite3_prepare_v2(db, query, -1, &stmt, nullptr) != SQLITE_OK) {
    Error() << "MBTILES: Failed to prepare SQL statement: "
            << sqlite3_errmsg(db);
    return tileData;
  }
...
FileSystemTiles::listDirectory(const std::string &path) const {
...
const std::vector<unsigned char> &
FileSystemTiles::getTile(int z, int x, int y, std::string &contentType) {
  contentType.clear();
  tileData.clear();
  ...
  for (const TileExt &t : tile_exts) {
    std::ifstream file((base + t.ext).c_str(),
...
std::string MapTiles::json(bool overlay) const {
  std::string out;
  JSON::Writer w(out);
  w.beginObject()
      .kv("id", layerID)
      .kv("name", name)
      .kv("attribution", attribution)
      .kv("overlay", overlay)
      .kv("url", "tiles/" + layerID + "/{z}/{x}/{y}")
      .kv("minZoom", minZoom)
      .kv("maxZoom", maxZoom)
      .key("resolutions")
      .beginArray();
...
#ifdef HASSQLITE
std::vector<double> MBTilesSupport::resolutions() const {
  std::vector<double> result;
  for (int z : zoomMapping)
    result.push_back(156543.03392804097 / (1 << z));
  return result;
}
#endif
