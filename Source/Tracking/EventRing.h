...
// vessel's record changing, each stamped with a place. One ring per level, so
// routine events never push out a distress call. The same words from the same
// sender while the earlier event is live count on that event: a device left
// transmitting for days is one event that began days ago.
class EventRing {
public:
  enum Kind : uint8_t {
    SAFETY = 1,
  ...
  static std::string formatText(const Event &e) {
    ...
    if (e.kind == PLACE_ENTER || e.kind == PLACE_EXIT)
      return out +
             (e.kind == PLACE_ENTER ? " · ::entered:: [[" : " · ::left:: [[") +
             escapeText(e.text) + "]]";
    if (!e.label.empty())
      return out + " · ::" +
  ...
  void push(Event e) {
    ...
    for (Event &have : r)
      if (e.kind != PLACE_ENTER && e.kind != PLACE_EXIT && have.kind == e.kind &&
