...
inline int BinaryStore::process(const JSON::JSON &data, FLOAT32 sender_lat, FLOAT32 sender_lon)
{
	...
	// a located item stands on its own marker unless it sits on its sender; anything
	// else belongs to a ship (addressee or sender); the sent chain holds it either way
	Item &b = items[h];
	if (b.sent_by != b.mmsi)
	{
		unlinkFromSent(h);
