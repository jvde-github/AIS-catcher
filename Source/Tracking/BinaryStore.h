...
public:
  struct Item {
    ...
    // identity scope, not the transmitter (that is mmsi): zero for sensor kinds
    // so copies via different towers merge
    uint32_t sender = 0;
    uint64_t hash = 0;
    // identity on the wire and in the table: 53 bits so JavaScript keeps it
    // exact
    uint64_t key = 0;
    uint64_t seq = 0;
  ...
  static bool aboutSender(Item::Kind k) {
    return k != Item::TEXT && k != Item::SAFETY && k != Item::LOCK &&
           k != Item::AREA && k != Item::SIGNAL;
  }
  ...
  // An item without a marker badges on the ship it is about: the addressee when
  // known, else the sender; a status report is about its transmitter. `known`
  // says whether an MMSI is in the table.
  template <typename Known> void settle(int h, Known known) {
    Item &b = items[h];
    uint32_t owner = 0;
    if (b.marker < 0)
      owner = (b.anchor && !aboutSender(b.kind) && known(b.anchor)) ? b.anchor
                                                                    : b.mmsi;
    if (b.owner == owner)
      return;
    unlinkFromOwner(h);
  ...
  void writeJSON(JSON::Writer &w, std::time_t now, std::time_t since,
                 uint64_t marker_key = 0, uint32_t owner_mmsi = 0) const;
  // markers changed after `since` (a change sequence, not a time), and the
  // removals; the caller refreshes first and writes the document around them
  ...
  template <typename F> void forEachMarker(std::time_t now, F f) {
    refresh(now);
...
private:
  ...
  static uint64_t hashPosition(uint64_t h, FLOAT32 lat, FLOAT32 lon) {
    return hashInt(
        hashInt(h, (int32_t)(lat * 10000 + (lat < 0 ? -0.5f : 0.5f))),
  ...
  static KeyKind classify(int &key, float &scale) {
    scale = 1;
    switch (key) {
    case AIS::KEY_WIND_SPEED_AVG:
      key = AIS::KEY_WSPEED;
      return BK_VALUE;
    case AIS::KEY_WIND_GUST_SPEED:
      key = AIS::KEY_WGUST;
      return BK_VALUE;
    case AIS::KEY_WIND_DIRECTION_AVG:
      key = AIS::KEY_WDIR;
      return BK_VALUE;
    case AIS::KEY_AIR_TEMPERATURE:
      key = AIS::KEY_AIRTEMP;
      return BK_VALUE;
    case AIS::KEY_DEW_POINT:
      key = AIS::KEY_DEWPOINT;
      return BK_VALUE;
    case AIS::KEY_BAROMETRIC_PRESSURE:
      key = AIS::KEY_PRESSURE;
      return BK_VALUE;
    case AIS::KEY_WATER_TEMPERATURE:
      key = AIS::KEY_WATERTEMP;
      return BK_VALUE;
    case AIS::KEY_VISIBILITY_KM:
      key = AIS::KEY_VISIBILITY;
      scale = 1 / 1.852f;
      return BK_VALUE;
    case AIS::KEY_WSPEED:
    case AIS::KEY_WGUST:
    ...
    case AIS::KEY_LON:
    case AIS::KEY_SEQNO:
    case AIS::KEY_RETRANSMIT:
    case AIS::KEY_STATION_ID:
    case AIS::KEY_DEST_MMSI:
    case AIS::KEY_ACK_REQUIRED:
    case AIS::KEY_LINKAGE_ID:
    case AIS::KEY_STATION_NAME:
    case AIS::KEY_NAME:
    case AIS::KEY_SITE_ID:
    case AIS::KEY_REPORT_TYPE:
    case AIS::KEY_UTC_DAY:
    case AIS::KEY_UTC_HOUR:
    case AIS::KEY_UTC_MINUTE:
    case AIS::KEY_UN_COUNTRY:
    case AIS::KEY_UN_LOCODE:
    case AIS::KEY_FAIRWAY_SECTION:
    case AIS::KEY_TERMINAL_CODE:
    case AIS::KEY_FAIRWAY_HECTOMETRE:
    case AIS::KEY_TUGBOATS:
    case AIS::KEY_AIR_DRAUGHT:
    case AIS::KEY_BERTH_TYPE:
    case AIS::KEY_BERTH_NUMBER:
    case AIS::KEY_BERTH_ARRIVAL_TIME:
    case AIS::KEY_BERTH_DEPARTURE_TIME:
    case AIS::KEY_BERTH_LAT:
    case AIS::KEY_BERTH_LON:
      return BK_META;
    case AIS::KEY_ASM_VOLTAGE_DATA:
    case AIS::KEY_ASM_CURRENT_DATA:
    case AIS::KEY_ASM_POWER_SUPPLY_TYPE:
    case AIS::KEY_ASM_LIGHT_STATUS:
    case AIS::KEY_ASM_BATTERY_STATUS:
    case AIS::KEY_ASM_OFF_POSITION_STATUS:
    case AIS::KEY_ANA_INT:
    case AIS::KEY_ANA_EXT1:
    case AIS::KEY_ANA_EXT2:
    case AIS::KEY_RACON:
    case AIS::KEY_HEALTH:
    case AIS::KEY_STAT_EXT:
    case AIS::KEY_OFF_POSITION:
      return BK_ATON;
    case AIS::KEY_TRAFFIC_SIGNAL:
    case AIS::KEY_NEXT_SIGNAL:
      return BK_SIGNAL;
    case AIS::KEY_AREA_NOTICE_TYPE:
    case AIS::KEY_AREA_NOTICE_NAME:
    case AIS::KEY_AREA_NOTICE_DURATION:
    case AIS::KEY_AREA_NOTICE_LAT:
    case AIS::KEY_AREA_NOTICE_LON:
    ...
    case AIS::KEY_STATION_TYPE:
    case AIS::KEY_SHIPTYPE:
    case AIS::KEY_SHIPTYPE_TEXT:
    case AIS::KEY_TXRX:
    case AIS::KEY_INTERVAL:
    case AIS::KEY_QUIET:
      return BK_META;
    case AIS::KEY_TEXT:
      return BK_TEXT;
    ...
    case AIS::KEY_LOCK_SCHEDULE:
    case AIS::KEY_VESSEL_NAME:
    case AIS::KEY_LAST_LOCATION:
    case AIS::KEY_LAST_ATA:
    case AIS::KEY_FIRST_LOCK:
    case AIS::KEY_FIRST_LOCK_ETA:
    case AIS::KEY_SECOND_LOCK:
    case AIS::KEY_SECOND_LOCK_ETA:
    case AIS::KEY_DELAY_LOCK:
    case AIS::KEY_RTA:
    case AIS::KEY_ETA:
    case AIS::KEY_LOCK_STATUS:
    case AIS::KEY_TIDAL:
  ...
  // the record of a ship, made when it has none; a recycled record's items lose
  // their ship
...
inline int BinaryStore::process(const JSON::JSON &data, FLOAT32 sender_lat,
                                FLOAT32 sender_lon, int ack_index) {
  const AIS::Message *msg = (AIS::Message *)data.binary;
  int type = msg->type();
  if (type != 6 && type != 7 && type != 8 && type != 12 && type != 13 &&
      type != 14 && type != 23)
    return -1;
  ...
  Item item;
  if (ack)
    item.anchor = msg->getUint(offset, 30);
  item.type = type;
  item.mmsi = msg->mmsi();
  std::string name;
  int linkage = 0;
  ...
  FLOAT32 ne_lat = LAT_UNDEFINED, ne_lon = LON_UNDEFINED,
          sw_lat = LAT_UNDEFINED, sw_lon = LON_UNDEFINED;

  {
    ...
    for (const auto &p : data.getMembers()) {
      ...
      case AIS::KEY_MESSAGE_ID:
        ...
        break;
      case AIS::KEY_LAT:
        item.lat = val.getFloat();
        break;
      case AIS::KEY_LON:
        item.lon = val.getFloat();
        break;
      case AIS::KEY_DEST_MMSI:
        item.anchor = (uint32_t)val.getInt();
        break;
      case AIS::KEY_LINKAGE_ID:
        linkage = (int)val.getInt();
        break;
      case AIS::KEY_AREA_NOTICE_LAT:
        item.lat = val.getFloat();
        break;
    ...
    if (ack) {
      // One retained record per acknowledged recipient/sequence pair. This
      // preserves all four recipients on the existing owner/sent chains.
      w.kv("dest_mmsi", item.anchor)
          .kv("ack_seqno", msg->getUint(offset + 30, 2));
      best = BK_TEXT;
    }
    w.endObject();
  }
  ...
  if (ack || type == 6 || type == 12) {
    // Addressed messages from different transmitters/sequence numbers must
    // not merge, even when their text or application data is identical.
    item.hash =
        hashInt(hashInt(item.hash, type),
                ack ? msg->getUint(offset + 30, 2) : msg->getUint(38, 2));
    item.sender = item.mmsi;
    // An acknowledgement with a reused two-bit sequence is a new event.
    if (ack)
      item.hash = hashInt(item.hash, (int32_t)msg->getRxTimeUnix());
  }
  if (item.kind == Item::AREA)
    item.sub = linkage;
  // two corners become the rectangle record an area notice carries; the item
  // stands at the centre
  if (item.kind == Item::AREA && item.shapes.empty() &&
      isValidCoord(ne_lat, ne_lon) && isValidCoord(sw_lat, sw_lon) &&
      ne_lat > sw_lat && ne_lon > sw_lon) {
    item.lat = (ne_lat + sw_lat) / 2;
    item.lon = (ne_lon + sw_lon) / 2;
    double east = (ne_lon - sw_lon) * 111320.0 *
                  std::cos(sw_lat * 3.14159265358979323846 / 180.0);
    double north = (ne_lat - sw_lat) * 111320.0;
    char rect[96];
    snprintf(rect, sizeof(rect), "r,%.5f,%.5f,%.0f,%.0f,0", sw_lon, sw_lat,
             east, north);
    item.shapes = rect;
  }
  ...
  // the transmitter is part of the identity for what is about the transmitter,
  // and for anything with neither a name nor a place; a placed sensor reading
  // merges across towers
  if (aboutSender(item.kind) ||
      (name.empty() && !isValidCoord(item.lat, item.lon)))
    item.sender = item.mmsi;

  item.key = (item.hash ^ ((uint64_t)item.kind << 48) ^
              ((uint64_t)(item.dac & 0x3FF) << 38) ^
              ((uint64_t)(item.fi & 0x3F) << 32) ^
              ((uint64_t)(uint16_t)item.sub << 24) ^ item.sender ^
              ((uint64_t)item.anchor << 8)) &
             0x1FFFFFFFFFFFFFULL;
  if (!item.key)
    item.key = 1;
  ...
  {
    size_t a = item.label.find_first_not_of(' '),
  ...
  // anything else belongs to a ship (addressee or sender); the sent chain holds
  // it either way
  Item &b = items[h];
  if (b.sent_by != b.mmsi) {
    unlinkFromSent(h);
  ...
  bool onSender =
      located && isValidCoord(sender_lat, sender_lon) &&
...
inline void BinaryStore::writeJSON(JSON::Writer &w, std::time_t now,
                                   std::time_t since, uint64_t marker_key,
                                   uint32_t owner_mmsi) const {
  w.beginObject()
      .kv("time", now)
      .kv("timeout", ttl)
      .key("messages")
  ...
  auto row = [&](const Item &b) {
    w.beginObject()
        .kv("key", (long long)b.key)
        .kv("seq", (long long)b.seq)
        .kv("type", b.type)
        .kv("dac", b.dac)
        .kv("fi", b.fi)
        .kv("timestamp", (long long)b.last)
        .kv("first", (long long)b.first)
        .kv("count", b.count)
        .kv("ttl", ttl)
        .kv("sender", b.mmsi);
    if (b.anchor)
  ...
  if (marker_key) {
    ...
    if (c != owners.NIL) {
      ...
      for (int p = owners[c].head; p >= 0; p = items[p].onext)
        if ((long int)now - (long int)items[p].last <= ttl)
          rows.push_back(p);
      for (int p = owners[c].sent; p >= 0; p = items[p].snext)
        if (items[p].owner != owner_mmsi &&
            (long int)now - (long int)items[p].last <= ttl)
          rows.push_back(p);
      std::sort(rows.begin(), rows.end(),
                [&](int a, int b) { return items[a].last > items[b].last; });
    ...
    items.forEach([&](int h) {
      ...
      if ((long int)now - (long int)b.last > ttl ||
          (since > 0 && b.last < since))
        return false;
      if (b.marker < 0)
        row(b);
      return true;
...
inline void BinaryStore::refresh(std::time_t now) {
  ...
  markers.forEach([&](int mh) {
    ...
    bool updated = m.count != count || m.last != nw.last ||
                   m.label != nw.label || m.senders != senders ||
                   m.shapes != nw.shapes;
    if (m.fresh || moved || updated) {
      m.flags =
          m.fresh ? Marker::NEW : (moved ? Marker::MOVED : Marker::UPDATED);
      m.fresh = false;
      m.kind = nw.kind;
      m.lat = nw.lat;
      m.lon = nw.lon;
  ...
  // ship chains shed their expired items too, or a busy station's chain grows
  // until its slots recycle
  auto expired = [&](int p) {
    return (long int)now - (long int)items[p].last > ttl;
  };
...
inline void BinaryStore::writeMarkerRows(JSON::Writer &w,
  ...
  markers.forEach([&](int mh) {
    ...
    w.beginObject()
        .kv("id", id)
        .kv("kind", (int)m.kind)
        .kv("flags", (int)m.flags)
        .kv("seq", (long long)m.seq)
        .kv("lat", m.lat)
        .kv("lon", m.lon)
        .kv("label", m.label)
        .kv("count", m.count)
        .kv("t", (long long)m.last)
        .kv("first", (long long)m.first)
        .kv("ttl", ttl);
    if (!m.shapes.empty())
