...
// Tracking/DB config accumulated during Set(), applied to all ReceiverTrackers
// in attachEngine().
struct TrackingConfig {
  ...
  int track_memory = 0;
  // ship table size; 0 keeps the default (4096, or 32 times that in server
  // mode)
  int max_ships = 0;
};

// Bundles all per-receiver (or aggregate) state: ship DB, counters, history.
// states[0] is always "All" (aggregate). states[1..N] are per receiver/model
// pair when there is more than one.
...
public:
  std::string label;
  // Device::getIdentity() plus the model, so attachEngine() can hand a tracker
  // back to the same input after a restart. Never shown: label is what the user
  // sees.
  std::string key;
  // Parallel, one entry per device; model_name lists that device's engines,
  // newline-separated.
  ...
  void setStationPosition(float lat, float lon, bool use_gps) {
    ships.setConfigPosition(lat, lon, use_gps);
  }
  ...
  void connectJSON(Connection<JSON::JSON> &c) {
    c.Connect((StreamIn<JSON::JSON> *)&ships);
  }
  void connectGPS(Connection<AIS::GPS> &c) {
    c.Connect((StreamIn<AIS::GPS> *)&ships);
  }
  void connectControl(Connection<AIS::Control> &c) {
    c.Connect((StreamIn<AIS::Control> *)&ships);
  }

  // Connect outgoing sinks (ships as source)
  template <typename T> void connectSink(T &sink) { ships >> sink; }
  ...
  std::string getShipsJSONcompact(std::time_t since = 0,
                                  const std::string &configVersion = "") {
    return ships.getJSONcompact(false, since, configVersion);
  }
  std::string getShipsTableJSON(std::time_t since = 0) {
    return ships.getJSONtable(since);
  }
  std::string getBinaryMessagesJSON(std::time_t since = 0, uint64_t marker = 0,
                                    uint32_t owner = 0) {
    return ships.getBinaryMessagesJSON(since, marker, owner);
  }
  std::string getMapObjectsJSON(uint64_t since = 0) {
    return ships.getMapObjectsJSON(since);
  }
  std::string getObjectJSON(const std::string &key) {
    return ships.getObjectJSON(key);
  }
  ...
  std::string getPathGeoJSON(uint32_t mmsi) {
    return ships.getPathGeoJSON(mmsi);
  }
  ...
  std::string getChangesJSON(uint32_t mmsi) {
    return ships.getChangesJSON(mmsi);
  }
};
