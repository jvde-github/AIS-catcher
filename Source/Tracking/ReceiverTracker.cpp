...
void ReceiverTracker::applyConfig(const TrackingConfig &cfg,
                                  const AIS::Filter &f) {
...
bool ReceiverTracker::save(std::ofstream &f, bool include_ships) {
  return counter.Save(f) && hist_second.Save(f) && hist_minute.Save(f) &&
         hist_hour.Save(f) && hist_day.Save(f) &&
         (!include_ships || ships.Save(f));
}

bool ReceiverTracker::load(std::ifstream &f) {
  if (!counter.Load(f) || !hist_second.Load(f) || !hist_minute.Load(f) ||
      !hist_hour.Load(f) || !hist_day.Load(f))
    return false;

  if (f.peek() != EOF && !ships.Load(f))
    Warning() << "Backup: ship positions could not be restored from backup "
                 "(format change?).";
...
static std::string orDash(const std::string &s) { return s.empty() ? "-" : s; }

// How a device names itself. attachEngine() needs this before it has a tracker
// to put it in, because the name is what identifies a tracker across engine
// runs.
