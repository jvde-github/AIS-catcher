...
// What changed about a vessel rather than where it went: one block per ship,
// claimed on its first change and returned when the slot is recycled. A field
// changing again inside its window is the same episode (new value, same onset);
// a numeric pair back where it began is no change; a draught baseline is kept
// once so the chart has a start.
...
public:
  ...
  void addNumeric(int ptr, Field field, uint8_t from, uint8_t to,
                  std::time_t now, bool initial = false) {
  ...
  void addEta(int ptr, uint8_t month, uint8_t day, uint8_t hour, uint8_t minute,
              std::time_t now) {
    if (!slot(ptr))
      return;
  ...
  void writeEntries(JSON::Writer &w, int ptr) const {
...
private:
  ...
  static bool numeric(uint8_t field) {
    return field == DRAUGHT || field == STATUS;
  }
  ...
  void add(int ptr, const Entry &in) {
    ...
    const uint32_t age = prev ? (in.time > prev->first ? in.time - prev->first
                                                       : prev->first - in.time)
                              : 0;
