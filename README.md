...
Some meta data is not calculated by default (to keep it light for running on mini computers as server) but can be activated with the ```-M``` switch. WE have signal power (in dB) and applied frequency correction (in ppm) activated with option ``-M D``. NMEA messages are timestamped with option ``-M T``. The examples above assume that these calculations are activated. 
...
As an example on how to further process some of the meta data generated by AIS-catcher, we used a few lines of JavaScript and Python that take the JSON output (generated with switch ``-o 3 -M D``) and use that to plot the location of ships on a map as a circle (Channel A/B corresponds to Blue/Red) 
...
If you are using a dongle that suffers from thermal drift (i.e. the required PPM corrections drifts if the dongle is getting warmer), you could consider to use the option ``-o CGF_WIDE on`` or switch to a FM decoding model which is less sensitive for frequency offsets (``-m 0``).
