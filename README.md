...
### Recorded signals
The `rtl_tcp` functionality provides a route to compare different receiver packages on a deterministic input from a file. I have tweaked the callback function in `rtl_tcp` so that it sends over input from a file to an AIS receiver like `AIS-catcher` and `AISrec`. The same can be easily done for `rtl-ais`. For both programs the sampling rate of the input file was converted using `sox` to 240K samples/second for `rtl-tcp` and 1.6M samples/second for `rtl-ais`. The output of the various receivers was sent via UDP to AISdipatcher which removes any duplicates. The results in terms of  number of messages/distinct vessels:
