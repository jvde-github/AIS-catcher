...
```
There is an option to GZIP the content (``GZIP on``), to provide an authentication username/password (``USERPWD username:password``), set the update interval in seconds (``INTERVAL 30``) and to switch off the display of the server response (``RESPONSE off``). 
...
AIS-catcher -H http://aprs.fi/jsonais/post/secret-key ID callsign PROTOCOL aprs INTERVAL 30 -q
```
Where ``secret-key`` should be your password and ``callsign`` your callsign.  The protocol setting ensures that AIS-catcher submits JSON in a form that is accepted by APRS.fi. 
I am also using this functionality to feed the map of [Chaos Consulting](https://adsb.chaos-consulting.de/map/) which accepts AIS-catcher output:
```
AIS-catcher -H https://ais.chaos-consulting.de/shipin/index.php USERPWD "Station:Password" GZIP on INTERVAL 5
```
Notice that this server requires authentication and accepts JSON with gzip encoding.
