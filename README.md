...
```
Do not use spaces in the comma seperated message type list. This only works with the filter switched to ``ON`` (default ``OFF``) and the filter needs to be defined per ``-u`` switch (or ``-H`` and ``-o``).
