	...
	try {
		...
		while (ptr < argc) {
			...
			case 'y':
				...
				if (count == 1)
					_receivers.back()->SpyServer().Set("host", arg1);
				if (count == 2)
			...
			case 'P':
				...
				break;
			case 'Q':
				Assert(count % 2 == 0, param, "invalid number of arguments");
				{
					msg.push_back(std::unique_ptr<IO::OutputMessage>(new IO::MQTTStreamer()));
					IO::OutputMessage& p = *msg.back();
					if (count >= 2)
						parseSettings(p, argv, ptr, argc);
				}
				break;
			case 'X':
		...
		if (list_options)
			Usage();
		if (list_devices || list_support || list_options)
			return 0;
