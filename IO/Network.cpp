...
namespace IO {
	...
	void MQTTStreamer::publish(const std::string& msg) {
		...
		if (!connected) {
			if (!connect()) {
				Error() << "MQTT: Failed to reconnect";
				return;
			}
		}
	...
	void MQTTStreamer::Start() {
		...
		ss << ", QoS: " << qos
		   << ", retain: " << Util::Convert::toString(retain);

		Info() << ss.str();

		if (!connect()) {
			throw std::runtime_error("MQTT: Failed to connect to broker");
		}
	}

	void MQTTStreamer::Stop() {
		if (connected) {
			// Send DISCONNECT packet, TO DO
			connected = false;
		}
		tcp.disconnect();
